...
class PresupuestoForm(forms.ModelForm):
    ...
    moneda_pres = forms.ChoiceField(
        ...
        help_text="Seleccione la moneda en la que se hará el presupuesto originalmente.",
    ...
    class Meta:
        model = Presupuesto
        fields = '__all__'
        exclude = ['id_pres', 'id_recibo', 'tipo_pres']

    def clean(self):
        cleaned_data = super().clean()
        monto_bs = cleaned_data.get("monto_pres_bs")
        monto_dl = cleaned_data.get("monto_pres_dl")
        moneda = cleaned_data.get("moneda_pres")
        ...
        if not cleaned_data.get('moneda_pres'):
            self.add_error('moneda_pres', "La selección de la moneda es obligatoria.")
        ...
        if not monto_bs and not monto_dl:
            self.add_error('monto_pres_bs', "Debe ingresar al menos un monto en Bolívares o en Dólares.")
            self.add_error('monto_pres_dl', "Debe ingresar al menos un monto en Bolívares o en Dólares.")

        if monto_bs and not monto_dl and moneda != 'bs':
            self.add_error('moneda_pres', "La moneda debe ser 'bs' si solo hay monto en Bolívares.")

        if monto_dl and not monto_bs and moneda != '$':
            self.add_error('moneda_pres', "La moneda debe ser '$' si solo hay monto en Dólares.")
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['monto_pres_bs'].label = 'Monto en Bolívares'
        self.fields['monto_pres_dl'].label = 'Monto en Dólares'
        self.fields['moneda_pres'].label = 'Tipo de Moneda'
